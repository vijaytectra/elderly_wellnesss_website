...
/* ==========================================================================
   MEDICAL DISCLAIMER BOX & COMMENT HIDE (PRESERVING ORIGINAL DESIGN 100%)
   ========================================================================== */
.comments-area,
#comments,
#respond,
...
def process_file(filepath):
    ...
    # Inject disclaimer right before <footer class="entry-meta">
    if '<div class="ew-medical-disclaimer">' not in content:
        if '<footer class="entry-meta">' in content:
            content = content.replace('<footer class="entry-meta">', DISCLAIMER_HTML + '\n<footer class="entry-meta">')
    ...
    if content != orig:
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"Injected Medical Disclaimer into: {filepath}")

def update_css():
    css_files = ["css/style.css", "css/blog-pages.css", "css/ew-a11y.css"]
    for c in css_files:
        if os.path.exists(c):
            with open(c, 'r', encoding='utf-8', errors='ignore') as f:
                c_content = f.read()
            if "MEDICAL DISCLAIMER BOX & COMMENT HIDE" not in c_content:
                c_content += DISCLAIMER_CSS
                with open(c, 'w', encoding='utf-8') as f:
                    f.write(c_content)
                print(f"Appended disclaimer CSS to: {c}")
